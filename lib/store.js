...
/*
 * Beyond the four, a document may be named by its path: `inat/120443.json`
 * is the iNaturalist archive for one taxon. Same read and write, same version
 * token, same conditional put — the only difference is that the caller says
 * where it lives. The path is checked rather than trusted, because a taxon id
 * arrives in a query string and this is the one place it becomes a filename.
 */
const DOC_PATH = /^[A-Za-z0-9_-]+(?:\/[A-Za-z0-9_-]+)*\/[A-Za-z0-9_-]+\.json$/;

/** The object path for a key: one of the four names, or a path of its own. */
function locate(key) {
  if (KEYS[key]) return KEYS[key];
  if (DOC_PATH.test(key)) return key;
  throw new Error(`not a document the store knows: ${key}`);
}
...
function fileStore(root, photoDir) {
  const pathFor = (key) => path.join(root, locate(key));
...
function s3Store(cfg) {
  ...
  async function read(key) {
    const object = locate(key);
    const res = await call('GET', urlFor(object));
    if (res.status === 404) return { value: undefined, token: null };
    if (res.status === 403) throw denialAdvice('GET', object, await res.text());
    if (!res.ok) throw new Error(`S3 GET ${object} -> ${res.status} ${await res.text()}`);
    return { value: JSON.parse(await res.text()), token: res.headers.get('etag') };
  }

  async function write(key, value, token) {
    const object = locate(key);
    const res = await call('PUT', urlFor(object), {
    ...
    if (res.status === 403) throw denialAdvice('PUT', object, await res.text());
    if (!res.ok) throw new Error(`S3 PUT ${object} -> ${res.status} ${await res.text()}`);
...
module.exports = {
  createStore, StoreConflict, KEYS, PHOTO_PREFIX, DOC_PATH, locate, serialize, sign, escapePath,
};
